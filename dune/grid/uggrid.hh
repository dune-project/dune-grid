  ...
  protected:
    ...
    static int ugGather_(typename UG_NS<dim>::DDD_OBJ obj, void* data)
    {
      if (codim == 0) {
        ...
        UGMakeableEntity<0, dim, UGGrid<dim> > e(static_cast<typename UG_NS<dim>::Element*>(obj),nullptr);
        // safety check to only communicate what is needed
        if ((level == -1 && UG_NS<dim>::isLeaf(static_cast<typename UG_NS<dim>::Element*>(obj))) || e.level() == level)
        {
        ...
        /** \bug The nullptr argument should actually the UGGrid object.  But that is hard to obtain here,
         * and the argument is (currently) only used for the boundarySegmentIndex method, which we don't call. */
        UGMakeableEntity<dim, dim, Dune::UGGrid<dim> > e(static_cast<typename UG_NS<dim>::Node*>(obj),nullptr);
        // safety check to only communicate what is needed
        if ((level == -1 && UG_NS<dim>::isLeaf(static_cast<typename UG_NS<dim>::Node*>(obj))) || e.level() == level)
        {
          ThisType msgBuf(static_cast<DataType*>(data));
          if (!duneDataHandle_->fixedsize(dim, codim))
            msgBuf.template writeRaw_<unsigned>(duneDataHandle_->size(e));
        ...
        /** \bug The nullptr argument should actually the UGGrid object.  But that is hard to obtain here,
         * and the argument is (currently) only used for the boundarySegmentIndex method, which we don't call. */
        UGMakeableEntity<dim-1, dim, Dune::UGGrid<dim> > e(static_cast<typename UG_NS<dim>::Edge*>(obj),nullptr);
        // safety check to only communicate what is needed
        if ((level == -1 && UG_NS<dim>::isLeaf(static_cast<typename UG_NS<dim>::Edge*>(obj))) || e.level() == level)
        {
          ThisType msgBuf(static_cast<DataType*>(data));
          if (!duneDataHandle_->fixedsize(dim, codim))
            msgBuf.template writeRaw_<unsigned>(duneDataHandle_->size(e));
    ...
    static int ugScatter_(typename UG_NS<dim>::DDD_OBJ obj, void* data)
    {
      if (codim == 0) {
        ...
        /** \bug The nullptr argument should actually the UGGrid object.  But that is hard to obtain here,
         * and the argument is (currently) only used for the boundarySegmentIndex method, which we don't call. */
        Entity e(static_cast<typename UG_NS<dim>::Element*>(obj),nullptr);
        // safety check to only communicate what is needed
        if ((level == -1 && UG_NS<dim>::isLeaf(static_cast<typename UG_NS<dim>::Element*>(obj))) || e.level() == level)
        {
          ThisType msgBuf(static_cast<DataType*>(data));
        ...
        /** \bug The nullptr argument should actually the UGGrid object.  But that is hard to obtain here,
         * and the argument is (currently) only used for the boundarySegmentIndex method, which we don't call. */
        Entity e(static_cast<typename UG_NS<dim>::Node*>(obj),nullptr);
        // safety check to only communicate what is needed
        if ((level == -1 && UG_NS<dim>::isLeaf(static_cast<typename UG_NS<dim>::Node*>(obj))) || e.level() == level)
        {
          ThisType msgBuf(static_cast<DataType*>(data));
        ...
        /** \bug The nullptr argument should actually the UGGrid object.  But that is hard to obtain here,
         * and the argument is (currently) only used for the boundarySegmentIndex method, which we don't call. */
        Entity e(static_cast<typename UG_NS<dim>::Edge*>(obj),nullptr);
        // safety check to only communicate what is needed
        if ((level == -1 && UG_NS<dim>::isLeaf(static_cast<typename UG_NS<dim>::Edge*>(obj))) || e.level() == level)
        {
          ThisType msgBuf(static_cast<DataType*>(data));
