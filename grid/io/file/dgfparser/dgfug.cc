  ...
  MacroGrid::Impl<UGGrid<dim,dimworld> >::generate(MacroGrid& mg,
    ...
    if (mg.readDuneGrid(gridin) == 1) {
      ...
      for (int n=0; n<mg.nofelements; n++) {
        ...
        for (int j=0; j<mg.elements[n].size(); j++)
          el.push_back((mg.elements[n][j]));
        if (el.size()==mg.dimw+1)
          ...
          std::cerr << "Worng no. of verticies for eleent" << std::endl;
      }
      grid->createEnd();
      return grid;
    }
    else {
      std::cerr << "Macrofile " << filename << " is not a valid DGF file\n"
                << "No alternative File-Format implemented" << std::endl;
      abort();
    }
  }
}
